#!/usr/bin/env python
### ARQUIVO ALTERADO PARA EXECUTAR A PWC-NET NO COLAB E SALVAR OS RESULTADOS EM FORMATO FACILMENTE COMPARTILHAVEL
import torch
import getopt
import math
import numpy
...
try:
	...
	sys.path.insert(0, '/content/EBSR/pwcnet/correlation'); import correlation # you should consider upgrading python
# end
...
for strOption, strArgument in getopt.getopt(sys.argv[1:], '', [ strParameter[2:] + '=' for strParameter in sys.argv[1::2] ])[0]:
	...
	if strOption == '--list' and strArgument != '': arguments_list = strArgument # path to to text file with format first second out
# end
...
if __name__ == '__main__':
	if arguments_list is None:
		...
		# save file
		stream = BytesIO()
		numpy.save(stream, tenOutput,  allow_pickle=True)
		objOutput.write(stream.getvalue())
		objOutput.close()
		print('finished...')
		
	else:
		list = open(arguments_list, 'r')
		for line in list:
			splited = line.split()

			tenFirst = torch.FloatTensor(numpy.ascontiguousarray(
				numpy.array(PIL.Image.open(splited[0]))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (
							1.0 / 255.0)))
			tenSecond = torch.FloatTensor(numpy.ascontiguousarray(
				numpy.array(PIL.Image.open(splited[1]))[:, :, ::-1].transpose(2, 0, 1).astype(
					numpy.float32) * (1.0 / 255.0)))

			tenOutput = estimate(tenFirst, tenSecond)

			# save file
			objOutput = open(splited[2], 'wb')
			stream = BytesIO()
			numpy.save(stream, tenOutput, allow_pickle=True)
			objOutput.write(stream.getvalue())
			objOutput.close()
			print(splited[2] + ' done!')
